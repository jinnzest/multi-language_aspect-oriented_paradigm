...
In simple words: business logic should be separated from aspects related to performance optimization, correctness, or other aspects such as connecting components to each other, allocating them in memory, or connecting software components to the outer world. There are no such concepts as computer memory or CPU from prospective of business logic. There are only transformations from input to output (arrows from one object to another in terms of category theory).

It leads us to the design when instead of single programming language we have a set of them. A main language expresses domain logic, while everything else is a set of aspects linked to the main language (item by item). These aspects add some nuances to the main language. The main language itself should be as simple as possible. Everything that can be moved to aspects should be moved to the aspects from the main language.

Separating aspects from the main language gives us good flexibility because now we can implement business logic postponing hardware-specific optimizations for a later time. Or we can quickly write code and only then run, for example, Big O complexity analysis to discover whether the algorithm is applicable in practice. Or run resources acquiring analysis for an algorithm like "sorting array" and catch, for example, that it creates some files (Obviously, it is not something a developer can expect from the such algorithm). Etc. 

Another consequence of separating domain logic from aspects: aspects may be chosen depending on needs. For example, in the case of quick prototyping, correctness and performance optimization-related analysis are not needed (Or too early in the case of making a production-ready product out of a prototype). Or when there are no requirements for high performance of a system then there is not need to waste time working on this. Etc. For sure, analytics a code generator needs can't be disabled, but there is no need for a developer to read these analytics. There should be some default directives to generate analyses without a developer intervention. 
...
The principle suggests a way to run analytics efficiently: to be able to modify a main language item without running a thorough analyzing down function calls hierarchy the main language items should be split into the smallest possible pieces (like functions, statements, expressions for example) and then each piece should be signed by a hash. It will allow using the memoization technique in a compiler to increase the speed of compiling a main language and aspects.   

When a parent function depends on subroutine functions then the source of a hash should be not only the parent function content but also hash codes of subroutine functions. So when down hierarchy any piece of code is changed hash code of related subroutine code will be changed so analytics should be run for the subroutine hierarchy and then for a parent function. In the case of generating a hash for an expression (like a function call, variable assignment, constant creation, etc) then all previous expressions the item depends on should be used as the source of the hash. For example when two different function calls use a variable as input then the variable's hash should be used as the source for each function call. 
